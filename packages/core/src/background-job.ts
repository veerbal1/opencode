...
type StartResult = { info: Info } | { info: Info; scope: Scope.Closeable; token: object; tail: Deferred.Deferred<void> }
      ...
      Effect.gen(function* () {
        ...
        const result = yield* SynchronizedRef.modifyEffect(
          ...
          Effect.fnUntraced(function* (jobs) {
            ...
            return [{ info: snapshot(job), scope, token, tail }, new Map(jobs).set(id, job)] as readonly [
              StartResult,
              Map<string, Active>,
            ]
          }),
        ...
        if ("scope" in result) {
          yield* fork(
            ...
            result.token,
            0,
            restore(input.run).pipe(Effect.ensuring(Deferred.succeed(result.tail, undefined))),
          )
        }
        return result.info
      }),
